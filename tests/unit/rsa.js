...
(function() {
  ...
  describe('rsa', function() {
    ...
    describe('bad data', function() {
      ...
      // NOTE: The DigestAlgorithm identifier checks exercised below were
      // added by the follow-up fixes for this issue (upstream commits
      // aa9372d, 56f4316 and 740954d). They close the remaining "invalid
      // structure but valid digest" cases: a substituted digest algorithm
      // identifier, and an md2/md5 AlgorithmIdentifier with its mandatory
      // NULL parameters stripped. As above, the signature values were
      // obtained by I^d mod N using the private exponent given above.

      function _checkBadDigestAlgorithm(publicKey, md, S) {
        ASSERT.throws(function() {
          publicKey.verify(md.digest().getBytes(), S);
        },
        /^Error: Unknown RSASSA-PKCS1-v1_5 DigestAlgorithm identifier.$/);
      }

      function _checkMissingNullParameters(publicKey, md, S) {
        ASSERT.throws(function() {
          publicKey.verify(md.digest().getBytes(), S);
        },
        /^Error: ASN.1 object does not contain a valid RSASSA-PKCS1-v1_5 DigestInfo value. Missing algorithm identifer NULL parameters.$/);
      }

      it('should check DigestAlgorithm identifier', function() {
        var publicKey = RSA.setPublicKey(N, e);
        // well formed DigestInfo carrying the correct SHA-256 digest of the
        // message, but announcing a digest algorithm that is not one of the
        // PKCS1-v1-5DigestAlgorithms of RFC 8017
        // (2.16.840.1.101.3.4.2.8 / SHA3-256 in place of
        ...
        var I = UTIL.binary.hex.decode(
          ...
          'ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff' +
          'ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff' +
          'ffffffffffffffffffffffff003031300d060960864801650304020805000420' +
          '7509e5bda0c762d2bac7f90d758b5b2263fa01ccbc542ab5e3df163be08e6ca9');
        var S = UTIL.binary.hex.decode(
          '9a0409a9ce7bb3a1a776e6ffcee5fb9a1aab94bf23c137980ec11aa6bcd56e27' +
          '2e2a77b1cc79bb86892d769a1f2933711c54757465c458a431ce3d68daf9965f' +
          'a52679cc9ff5021bb435ec652d6ca78d1e23c5c6a507ecd81e2e9abcdea2bc5b' +
          '23520f03e159a8db5b911e50ce748b00d738c4904505937d52119fbe9e66bfdf' +
          '83cd47a689a5ab1d8bb378643963350cae8dbd29ed0b9345e7dd7f08e54f28ce' +
          'dab707c7ac41882cd83ce185fc7ccd2c40a8f5c0fe93f54d124bcc58746ed361' +
          'bc4f74b63225d3d0ddbc9b74b5a2b6ccabb1914f1226f3b95ae47573595a768a' +
          '11deb6822bd3b79466f6ee0ccfb6b66414e94381c18c608b3578d469b52f29be');

        var md = MD.sha256.create();
        md.update(m);
        _checkBadDigestAlgorithm(publicKey, md, S);
      });

      it('should check md5 DigestAlgorithm NULL parameters', function() {
        var publicKey = RSA.setPublicKey(N, e);
        // DigestInfo carrying the correct MD5 digest of the message with the
        // NULL parameters of the AlgorithmIdentifier removed; RFC 8017
        // requires them to be present for md2 and md5
        var I = UTIL.binary.hex.decode(
          '0001ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff' +
          'ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff' +
          'ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff' +
          ...
          '301e300a06082a864886f70d02050410fc3ff98e8c6a0d3087d515c0473f8677');
        var S = UTIL.binary.hex.decode(
          'cffe4dcb8325139d57b6406bf28c04522417f97c68b758fa671180ede7b7fabb' +
          'f0213383f97eebc4ab1add3ad86f822c82dc56efc235738222f2ec72b7669551' +
          '9551b3a0c43385a66fc5ce36a1b4bf108165ab2ffd927652efde97bbd8ea415d' +
          '0e99207cfbfdfaffa8f6f7faf730c7f6ca9bba047406ff302199cec96eb6cc05' +
          'ac5f25615cfa1c6e2d068b462818a54a6c16765dea501a44c769cc0a04f05f73' +
          '03ef10f88e893f451364c0c1b87b98b0f535e02082595a0985348d56d850b42b' +
          '97a9b75b7684a8d4025c1ccd83a881c95c5f3003da70ea99cc36d62c8adcd7ba' +
          'cfaf8c6f1b30ef33dd20a40381373691fd742f6b810222330891bb5f6582260c');

        var md = MD.md5.create();
        md.update(m);
        _checkMissingNullParameters(publicKey, md, S);
      });

      it('should verify md5 DigestInfo with NULL parameters', function() {
        var publicKey = RSA.setPublicKey(N, e);
        // same as above with the mandatory NULL parameters left in place
        var I = UTIL.binary.hex.decode(
          '0001ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff' +
          'ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff' +
          'ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff' +
          ...
          '300c06082a864886f70d020505000410fc3ff98e8c6a0d3087d515c0473f8677');
        var S = UTIL.binary.hex.decode(
          '3694537ca97dcc93d9dcbd2f02f4899c8ddba42977e2eed7de78c20c36eaa407' +
          '65c5a3642d8b000a0082d174fa4840363e957b3cd7f6559709d3414ac6dd919c' +
          'f1d7eb8dc78566be43730b9b1d1c9e8be703368b9430964e2940330b82056988' +
          '3c4a648cfdfbd89e101b37686a0752518c7ddd1a65340ff7ca8c5d5f4dcdbf95' +
          'cfb894fc318f635f8cd8b62d3d8421c465af39f241cf0839fd26b686a4826c82' +
          '5963319b58de9630c5e5828ada394c0c0f8e5ed4126b8981152a25314a41f0b1' +
          '3faa7abe34f89d64b9b041f74541e3d1b4c32afa67d4fc68bdc50a831e42722d' +
          '331bdcaf35458462d6ee404920969987f132c64018c3e1b4193d5e7227751ce6');

        var md = MD.md5.create();
        md.update(m);
        ASSERT.ok(publicKey.verify(md.digest().getBytes(), S));
      });

      it('should verify sha512-256 DigestInfo', function() {
        var publicKey = RSA.setPublicKey(N, e);
        // one of the digest algorithms added to the accepted set
        ...
        var I = UTIL.binary.hex.decode(
          ...
          'ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff' +
          'ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff' +
          'ffffffffffffffffffffffff003031300d060960864801650304020605000420' +
          '595b5926068b4828fb1c27db21281e31118b8475cb6c3ceeb09be7b685414d5f');
        var S = UTIL.binary.hex.decode(
          '5ff4627de1e57ff1befcbe83e065db483f25e2d5c2bcbeb75867da566dda8730' +
          'e800251c877dc2a83b338568103cfa91a29ffa8840bf3269407d5e6bb8ef7049' +
          'a679f03b6753a20d0705e7ec2b23025ec0f87fc2193472440e12a75bb025a869' +
          'a106339fbf5758242253a64fd08050b436bad7f05fc17b378471ab821d8514cb' +
          '4eed40716fe972fc448fca06b683d68751450de4ba9640e5ea045ec59f5fcc54' +
          '2d018111415fa101fb03c185d9e2635983b219ca8edbad82152d418c4fa84baf' +
          '0024f76edf6805c5139a4d29917ffb6bbcaf8d8c6e18f2e795e0d7adbc87aaf1' +
          'df8d63f205e01586d3055140b0f72cceb41d302aa469739b0d1e2d3bd445156a');

        var md = MD['sha512/256'].create();
        md.update(m);
        ASSERT.ok(publicKey.verify(md.digest().getBytes(), S));
      });

      it('should know the RFC 8017 digest algorithm oids', function() {
        ASSERT.equal(FORGE.oids.md2, '1.2.840.113549.2.2');
        ASSERT.equal(FORGE.oids['1.2.840.113549.2.2'], 'md2');
        ASSERT.equal(FORGE.oids.sha224, '2.16.840.1.101.3.4.2.4');
        ASSERT.equal(FORGE.oids['2.16.840.1.101.3.4.2.4'], 'sha224');
        ASSERT.equal(FORGE.oids['sha512-224'], '2.16.840.1.101.3.4.2.5');
        ASSERT.equal(FORGE.oids['2.16.840.1.101.3.4.2.5'], 'sha512-224');
        ASSERT.equal(FORGE.oids['sha512-256'], '2.16.840.1.101.3.4.2.6');
        ASSERT.equal(FORGE.oids['2.16.840.1.101.3.4.2.6'], 'sha512-256');
      });
    });
    ...
    describe('GHSA-ppp5-5v6c-4jwp DigestInfo/padding checks', function() {
      ...
      it('should check forged data (via digestinfo length)', function() {
        ...
        new RegExp('^Error: ASN\\.1 object does not contain a valid ' +
