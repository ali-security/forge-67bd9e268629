    ...
    if(unused === 0) {
      try {
        ...
        var subOptions = {
          ...
          strict: true,
          decodeBitStrings: true,
          // keep enforcing the recursion limit inside encapsulated content
    ...
    if(obj.constructed === v.constructed ||
      ...
      if(v.value && forge.util.isArray(v.value)) {
        ...
        for(var i = 0; rval && i < v.value.length; ++i) {
          ...
          // if the schema explicitly specifies tagClass/type, do a quick
          // structural check to avoid unnecessary recursion/side-effects when
          // tags clearly don't match
          ...
          if(schemaHasTag &&
            ...
            // tags do not match
            if(schemaItem.optional) {
              // skip this schema element, do not consume objChild and do not
          ...
          // tags are compatible (or the schema did not declare tags), dive
          ...
          if(childRval) {
            ...
            // validation failed but the element is optional, skip the schema
            // item and do not consume the child
            rval = true;
          } else {
            // required item failed, errors should already be populated by the
            // recursive call; keep failing
            rval = false;
            break;
          }
        }
